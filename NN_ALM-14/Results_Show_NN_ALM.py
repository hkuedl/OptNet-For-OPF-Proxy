...
if __name__ == "__main__":
    ...
    Out_Pre_Test_PG = torch.load(f"Res_NN_ALM\\Pre_{SFX}_PG_NN_ALM.pt", map_location=torch.device('cpu'))
    Out_Pre_Test_QG = torch.load(f"Res_NN_ALM\\Pre_{SFX}_QG_NN_ALM.pt", map_location=torch.device('cpu'))
    Out_Pre_Test_VM = torch.load(f"Res_NN_ALM\\Pre_{SFX}_VM_NN_ALM.pt", map_location=torch.device('cpu'))
    Out_Pre_Test_VA = torch.load(f"Res_NN_ALM\\Pre_{SFX}_VA_NN_ALM.pt", map_location=torch.device('cpu'))
    #
    Out_Pre_Test_PGQG = torch.hstack((Out_Pre_Test_PG, Out_Pre_Test_QG))
    Out_Pre_Test_VMVA = torch.hstack((Out_Pre_Test_VM, Out_Pre_Test_VA))
